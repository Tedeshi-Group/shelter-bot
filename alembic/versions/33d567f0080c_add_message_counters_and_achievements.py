...
def upgrade() -> None:
    ...
    if not _table_exists(conn, 'achievement_levels'):
    ...
    if not _table_exists(conn, 'message_counters'):
    ...
    if not _column_exists(conn, 'users', 'total_messages'):
        op.add_column('users', sa.Column('total_messages', sa.Integer(), nullable=False, server_default='0'))

    # Seed achievement data (idempotent — skip if already populated)
    count = conn.execute(sa.text("SELECT COUNT(*) FROM achievements")).scalar()
    if count == 0:
